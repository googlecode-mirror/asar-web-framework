...
class Asar_ControllerTest extends Asar_Test_Helper {
    ...
    protected function _testForUndefinedMethodsReturn405($method) {
		...
		$this->assertEquals(
            ...
            "Controller did not return a response with expected 405 as status for method '$method'"
        );
	}
	
	protected function _classNameGenerator($aClass) {
	    do {
	...
	protected function _assertClassWasInstantiated($classname) {
	...
	function testGettingTheContext() {
	    
	    $this->markTestIncomplete();
	}
	
	/*
	function testRequestingWithSubPathsShouldReadAlistOfMappings() {
